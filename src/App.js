...
export default function App() {
  ...
  // 🔥 무기/장비용 비용 공식 (10배 적용)
  const calculateBaseCost = (lvl, startBase) => { const tier = Math.floor(lvl / 9); const step = (lvl % 9) + 1; return step * Math.pow(10, tier) * startBase * 10; };
  
  // 🔥 펫/성용 비용 공식 (10배 제외 -> 4억 대신 4천만으로 현실화)
  const calculatePetCastleCost = (lvl, startBase) => { const tier = Math.floor(lvl / 9); const step = (lvl % 9) + 1; return step * Math.pow(10, tier) * startBase; };
  ...
  const getPetCost = useCallback((lvl, nLvl, mult = hState.mult) => { return Math.floor(calculatePetCastleCost(lvl, 100) * (1 - (nLvl * 0.005)) * mult); }, [hState.mult]);
  const getCastleCost = useCallback((lvl, nLvl, mult = hState.mult) => { return Math.floor(calculatePetCastleCost(lvl, 1000) * (1 - (nLvl * 0.005)) * mult); }, [hState.mult]);
